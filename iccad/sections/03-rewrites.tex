...
\zz{we only need to talk about the implementation details in the evaluation section. Also avoid giving reviewers the false impression that we developed a full-featured verilog frontend from scratch}
\fixme{Our compiler has a Verilog frontend} for the sake of converting netlists to our
...
To that end, the semantics of a LUT is simply applying its boolean \zz{Boolean} inputs to the function:

\begin{equation}
    \llbracket \texttt{(LUT F x0 x1)} \rrbracket = F(\llbracket \texttt{x0} \rrbracket, \llbracket \texttt{x1} \rrbracket)
\end{equation}

\zz{clarify the meaning of $\llbracket$ $\rrbracket$}
...
are also handled by this rule. \todo{explain that we are eliminating terms that
    are not useful to rewrite} Since this rule is computationally expensive, it is
