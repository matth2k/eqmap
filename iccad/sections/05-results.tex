...
most cases \zz{can we be more specific, how many exactly finish within 30 sec},
...
\begin{figure}
    \begin{subfigure}{0.47\textwidth}
        ...
        \caption{Distribution of 128 LUTs after running \shortname{} on `i9' synthesized with Yosys 0.33.}\label{fig:histogram:y33}
    \end{subfigure}
    \hfill\vspace{4mm}
    \begin{subfigure}{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/y47.pdf}
        \caption{Distribution of 137 LUTs after running \shortname{} on `i9' synthesized with Yosys 0.47.}\label{fig:histogram:y47}
    \end{subfigure}
    \caption{Comparison of \shortname{}'s ability to optimize circuits given different starting structures.}\label{fig:histogram}
\end{figure}
...
\zz{Our evaluation on retiming and sequential logic is very weak. Maybe we should consider only focusing on comb logic in this paper. If not, we need to clarify why we are not testing more complex sequential circuits}
...
\todo{470 LUTs} with \shortname{}. Among the \nimproved{} improved benchmarks
in Table~\ref{tab:results}, six exceed 1000 LUTs. Although this e-graph driven
technology provides promising results for smaller benchmarks, FPGA technology
mapping is especially difficult---and important---for larger designs with tens
of thousands of LUTs. Through our case study on bit-parallel designs, we
demonstrate that \shortname{} performs just as well on large designs, achieving
area reductions without incurring excessive build times.
