...
highest level, FPGA and ASIC design flows can be split into \fixme{logical
    synthesis} and \fixme{physical synthesis} (e.g., floorplanning placement and
routing). \zz{logic synthesis and physical design -- check other places} This
...
acceptable QoR (quality of results) \zz{quality of results (QoRs) -- put
    acronym in the parenthesis}.

\fixme{With the end of Moore's Law scaling, chip area becomes a tighter constraint, and logic synthesis is more of a bottleneck}. \zz{rewrite -- Denard scaling is dead; Moore Law not done yet; chip area is not really a critical constraint (power is); also it's a stretch to say logic synthesis is a bottleneck; it's still a critical stage for sure} Hence, future synthesis tools will
...
Technology mapping is the hand-off between \fixme{logical synthesis and
    physical synthesis}. It converts the abstract Boolean logic into a network of
\fixme{gates} \zz{components -- use more generic term here since we want to
    cover both asic and fpga} that belong to the target cell library. For FPGAs,
the primary target cell is the LUT (lookup table) \zz{look-up table (LUT)}.
...
synthesis. \zz{there are older papers we should cite on boolean matching}
...
span the time-QoR spectrum. Equivalence graphs, \zz{equality graphs -- check
    the egg paper} referred to as e-graphs, are a data structure which use
union-find operations to compactly represent abstract equivalence relations.
E-graphs are useful as an \fixme{optimizing compiler framework}, \zz{e-graphs
    themselves are not frameworks; let's just say they are used to optimize
    programs ...} because terms can be iteratively rewritten in a nondestructive
...
\zz{we need to properly introduce packing before bringing up repacking. With the current writing, it's unclear how we perform tech mapping and packing}
